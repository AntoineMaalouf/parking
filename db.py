...
def _db_path() -> str:
    """Resolve DB path at call time so DATA_DIR can be set after import. (#9)"""
    return os.path.join(os.getenv("DATA_DIR", "."), "parking.db")
...
async def init_db() -> None:
    async with aiosqlite.connect(_db_path()) as db:
        await db.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_bay_store_updated_at
                ON bay_store (updated_at);
